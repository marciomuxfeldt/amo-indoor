...
// NOVA FUNÇÃO: Retry com exponential backoff
async function fetchWithRetry<T>(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      console.log(`📡 [${name}] Tentativa ${attempt}/${maxRetries}`)
      const result = await fetchFn()
      console.log(`✅ [${name}] Sucesso`)
      return result
    } catch (error) {
      console.error(`❌ [${name}] Falhou na tentativa ${attempt}:`, error)
      
      if (attempt === maxRetries) {
        throw new Error(`${name} falhou após ${maxRetries} tentativas: ${error instanceof Error ? error.message : 'Erro desconhecido'}`)
      }
      
      // Exponential backoff: 1s, 2s, 4s
      const delay = baseDelay * Math.pow(2, attempt - 1)
      console.log(`⏳ [${name}] Aguardando ${delay}ms antes de tentar novamente...`)
      await new Promise(resolve => setTimeout(resolve, delay))
    }
  }
  
  throw new Error(`${name} falhou após ${maxRetries} tentativas`)
}
...
async function initialize(): Promise<void> {
  ...
  console.log('🔑 Device Code:', storedDeviceCode)
  
  if (!storedDeviceId) {
    console.log('❌ Device ID não encontrado, redirecionando para pareamento')
    router.push({ name: 'tv-pairing' })
    return
  }

  if (!isValidUUID(storedDeviceId)) {
    console.log('❌ Device ID inválido, limpando e redirecionando')
    storage.removeLocalStorage('deviceId')
    storage.removeLocalStorage('deviceCode')
    router.push({ name: 'tv-pairing' })
    return
  ...
  try {
    ...
    // OTIMIZAÇÃO: Carregar dados progressivamente com retry
    // Aumentar timeout para 60 segundos
    const TIMEOUT_MS = 60000
    
    // Carregar devices e settings primeiro (mais crítico)
    console.log('1️⃣ Carregando devices e settings...')
    await Promise.race([
      Promise.all([
        fetchWithRetry(() => devicesStore.fetchDevices(), 'Devices'),
        fetchWithRetry(() => devicesStore.fetchSettings(), 'Settings')
      ]),
      new Promise((_, reject) => 
        setTimeout(() => reject(new Error(`Timeout: devices/settings demorou mais de ${TIMEOUT_MS / 1000}s`)), TIMEOUT_MS)
      )
    ])
    
    console.log('✅ Devices e settings carregados')

    // Verificar se o device existe
    const currentDevice = devicesStore.devices.find(d => d.id === storedDeviceId)
    ...
    if (storedDeviceCode && currentDevice.code !== storedDeviceCode) {
      console.log(`⚠️ Token mudou! Salvo: ${storedDeviceCode}, Atual: ${currentDevice.code}`)
      storage.removeLocalStorage('deviceId')
      storage.removeLocalStorage('deviceCode')
      
      configError.value = `Token desta TV foi alterado pelo administrador.\n\nCódigo anterior: ${storedDeviceCode}\nNovo código: ${currentDevice.code}\n\nEsta TV precisa ser pareada novamente com o novo código.\n\nRedirecionando para tela de pareamento em 5 segundos...`
    ...
    if (!storedDeviceCode) {
      storage.setLocalStorage('deviceCode', currentDevice.code)
    }

    // Carregar conteúdo (pedidos, produtos, mídias) em paralelo
    console.log('2️⃣ Carregando conteúdo (pedidos, produtos, mídias)...')
    await Promise.race([
      Promise.all([
        fetchWithRetry(() => ordersStore.fetchOrders(), 'Orders'),
        fetchWithRetry(() => mediaStore.fetchProducts(), 'Products'),
        fetchWithRetry(() => mediaStore.fetchMedia(), 'Media')
      ]),
      new Promise((_, reject) => 
        setTimeout(() => reject(new Error(`Timeout: conteúdo demorou mais de ${TIMEOUT_MS / 1000}s`)), TIMEOUT_MS)
      )
    ])
